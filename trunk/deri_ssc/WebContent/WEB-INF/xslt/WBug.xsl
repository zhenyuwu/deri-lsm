...
<xsl:stylesheet
   ...
   xmlns:aws="http://www.aws.com/aws">
	...
	<xsl:template match="aws:ob">				
		...
		<xsl:apply-templates select="aws:temp"/>		
		<xsl:apply-templates select="aws:current-condition"/>	
		<xsl:apply-templates select="aws:humidity"/>		
		<xsl:apply-templates select="aws:pressure"/>	
		<xsl:apply-templates select="aws:wind-direction"/>
		<xsl:apply-templates select="aws:wind-speed"/>
	</xsl:template>
	...
	<xsl:template match="aws:humidity">		
		<xsl:call-template name="hasUnit">
	...
	<xsl:template match="aws:pressure">		
		<xsl:call-template name="hasUnit">
	...
	<xsl:template match="aws:wind-direction">
		<xsl:call-template name="noUnit">
			<xsl:with-param name="name" select="'Direction'"/>
			<xsl:with-param name="value" select="node()" />
		</xsl:call-template>	
	</xsl:template>	
	<xsl:template match="aws:wind-speed">
		<xsl:call-template name="hasUnit">
